...
var move_speed = 250.0	# Global move speed (units per second)
var is_moving = false	# Boolean to check if the player is moving

var start_position = Vector2()
var end_position = Vector2()
var lerp_t = 0.0	# Interpolation weight (0.0 to 1.0)
...
func _physics_process(delta):
	if is_moving:
		lerp_t += delta * move_speed / start_position.distance_to(end_position)
		if lerp_t < 1.0:
